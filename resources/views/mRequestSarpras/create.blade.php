                            ...
                            <div class="intro-y box lg:mt-5">
                                <div class="flex items-center p-5 border-b border-gray-200 dark:border-dark-5">
                                    <h2 class="font-medium text-base mr-auto">
                                        Form Request Sarpras
                                    </h2>
                                </div>
                                <div class="p-5">
                                    <form action="{{ route('Try.store') }}" method="POST" enctype="multipart/form-data">
                                        @csrf
                                        <div class="grid-cols-12">
                                            <div class="col-span-12 xxl:col-span-6">
                                                <div>
                                                    <label for="update-profile-form-3" class="form-label">Ruangan</label>
                                                    <select id="ruang-kelas" name="ruang" id="update-profile-form-3" data-search="true" class="tail-select w-full">
                                                        <option value="">Pilih ruangan terlebih dahulu</option>
                                                        @foreach ( $data as $result )
                                                            <option value="{{ $result->id_data }}">{{ $result->list_data_ruangan }} </option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                            </div>
                                        ...
                                        <div class="flex gap-5 mt-5">
                                            <div class="col-span-12 xl:col-span-6 flex">
                                                ...
                                                <div class="flex-grow mt-3 mr-3">
                                                    <label for="update-profile-form-3" class="form-label">Harga Barang</label>
                                                    <input name="harga_barang[]" id="update-profile-form-3" type="text" class="form-control" placeholder="Input text">
                                                </div>
                                                <div class="flex-grow mt-3 mr-3">
                                                    <label for="update-profile-form-2" class="form-label">Keterangan Kebutuhan</label>
                                                    <input name="ket_barang[]" id="update-profile-form-2" type="text" class="form-control" placeholder="Input text">
                                                </div>
                                            </div>
                                        </div>   
                                        <div class="flex gap-5 mt-5">
...
<script type="text/javascript">
    ...
    $(function () {
        $('#ruang-kelas').change(function () {
            ...
            $.ajax({
                url: "{{ route('getExistedReq.data', '') }}/" + selectedValue,
                method: 'GET',
                    ...
                    $.each(data, function (index, item) {
                        // Buat elemen HTML untuk menampilkan data, misalnya dalam <div>
                            var html = '<div class="col-span-12 xl:col-span-6 flex">' +
                                        ...
                                        '<input name="harga_barang[]" value="' + item.harga_barang + '" id="update-profile-form-3" type="text" class="form-control" placeholder="Input text" disabled>' +
                                        '</div>' +
                                        '<div class="flex-grow mt-3 mr-3">' +
